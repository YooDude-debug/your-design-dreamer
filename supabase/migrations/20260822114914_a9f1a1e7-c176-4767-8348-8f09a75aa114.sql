...
DECLARE
  ...
  best_up int := 0;
BEGIN
  -- Idempotenz: Zeile sperren, bereits geschlossene oder noch laufende Runden ignorieren.
  ...
  SELECT COALESCE(MAX(up_count), 0) INTO best_up FROM public._tally;
  ...
  -- Gewinner in die Globe-Datenbank (keine Doppelanlage dank Unique-Indizes).
  ...
  -- Aktives Voting leeren: alle Teilnehmer dieser Runde (Gewinner, Verlierer, 0-Vote).
