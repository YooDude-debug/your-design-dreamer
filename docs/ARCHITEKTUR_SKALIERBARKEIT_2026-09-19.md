...
| **J Monitoring** | `ops_events`/`ops_incidents` in Postgres (damit instanzübergreifend korrekt), Unique-Index `(environment, fingerprint) WHERE status <> 'resolved'` verhindert Doppel-Vorfälle bei parallelen Instanzen, Alarmzustellung mit zwei Kanälen + Retry, externer Totmann-Schalter (`OPS_HEARTBEAT_URL`), automatisches Aufräumen (14 Tage), `/admin/health`, 5-Minuten-Health-Cron, Cache-Metrik-Endpunkt, Tests für Alerting | Laufzeitkennzahlen nur je Instanz und ohne Aggregation, kein verteiltes Tracing, Plattform-Log-Alarme nicht projektseitig klassifizierbar (siehe `RLS_PERMISSION_INCIDENT_2026-09-17.md`) |
