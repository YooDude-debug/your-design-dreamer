...
/**
 * "A word that doesn't exist in English…" – 10-Sekunden-Short (9:16, 300 Frames).
 *
 * 0–2 s Hook · 2–4 s Wort FERNWEH (+ deutscher SlangTag) ·
      ...
      <AbsoluteFill
        style={{
          background: `radial-gradient(1100px 900px at ${540 + drift}px ${
            740 - drift * 0.6
          }px, rgba(47,240,140,${0.06 + energy * 0.2}) 0%, rgba(5,7,6,0) 62%)`,
        }}
      />
      <AbsoluteFill
        style={{
...
const SceneHook: React.FC = () => {
  ...
  const eyes = spring({ frame: frame - 28, fps, config: { damping: 10, stiffness: 220 } });
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      >
        ...
        <div style={{ fontSize: 96, opacity: eyes, transform: `scale(${0.6 + eyes * 0.4})` }}>
          👀
        </div>
      </AbsoluteFill>
...
/** 2–4 s · Das Wort + hörbarer deutscher SlangTag. */
const SceneWord: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      ...
      <Sequence from={4}>
        <Audio src={staticFile("audio/de-fernweh.mp3")} volume={1} />
      </Sequence>
...
const SceneMeaning: React.FC = () => {
  ...
  const pan = interpolate(frame, [0, 90], [0, -60], {
    ...clamp,
  ...
  return (
    <AbsoluteFill style={{ opacity: out }}>
      <Backdrop energy={0.5} />
      {/* Reisesehnsucht: Horizont-Andeutung, rein grafisch */}
      <AbsoluteFill
        style={{
          transform: `translateX(${pan}px)`,
          background:
            "radial-gradient(760px 300px at 50% 74%, rgba(79,209,245,0.16) 0%, rgba(0,0,0,0) 70%)",
        }}
      />
      <div
        style={{
          ...
          top: 1240,
          height: 2,
          background: `linear-gradient(90deg, rgba(0,0,0,0) 0%, ${C.green}66 50%, rgba(0,0,0,0) 100%)`,
          opacity: glow,
        }}
      />
      ...
      >
        ...
        >
          A deep longing
          <br />
          to be <span style={{ color: C.green }}>somewhere else.</span>
        </div>
        <div style={{ fontSize: 92, opacity: glow }}>✈️🌍</div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
};
