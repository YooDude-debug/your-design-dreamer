...
const composition = await selectComposition({ serveUrl: bundled, id: "messenger-demo", puppeteerInstance: browser });
for (const f of process.argv.slice(2)) {
  await renderStill({ composition, serveUrl: bundled, output: `/tmp/msg-${f}.png`, frame: Number(f), puppeteerInstance: browser });
  console.log("frame", f);
}
