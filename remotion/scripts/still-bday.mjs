...
const composition = await selectComposition({ serveUrl: bundled, id: "birthday", puppeteerInstance: browser });
for (const f of process.argv.slice(2).map(Number)) {
  await renderStill({ composition, serveUrl: bundled, output: `/tmp/browser/bday_${f}.png`, frame: f, puppeteerInstance: browser });
  console.log("frame", f);
}
