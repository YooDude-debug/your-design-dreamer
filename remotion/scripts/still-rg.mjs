...
const composition = await selectComposition({ serveUrl: bundled, id: "feed-short", puppeteerInstance: browser });
for (const f of [180, 196, 210]) {
  await renderStill({ composition, serveUrl: bundled, output: `/tmp/rg-${f}.png`, frame: f, puppeteerInstance: browser, overwrite: true });
}
