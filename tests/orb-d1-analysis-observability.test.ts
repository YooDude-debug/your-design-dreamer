...
const run = () => analyzeContextWindow({ transcript: TRANSCRIPT, knownMemories: [MEMORY], allowedNodeIds: [] });
...
describe("D1 – Hintergrundanalyse Messwerte", () => {
  ...
  it("A: HTTP-Fehler → http, Status erfasst, 0/0", async () => {
    for (const status of [429, 401, 403, 400, 500]) {
      ...
      const r = await run();
      expect(r).toMatchObject({ failure: "http", httpStatus: status, preSanitizeCount: 0, candidates: [] });
    }
  });
  ...
  it("B: Timeout → timeout, httpStatus null", async () => {
    ...
    expect(await run()).toMatchObject({ failure: "timeout", httpStatus: null, preSanitizeCount: 0 });
  });
  ...
  it("C: leerer bzw. ungültiger Inhalt → malformed, Status 200", async () => {
    for (const c of [null, "", "{nicht json"]) {
      vi.stubGlobal("fetch", vi.fn(async () => okResponse(c)));
      expect(await run()).toMatchObject({ failure: "malformed", httpStatus: 200, preSanitizeCount: 0 });
    }
  });

  it("D: candidates [] → failure null, 0/0", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => okResponse(JSON.stringify({ candidates: [] }))));
    const r = await run();
  ...
  it("E: 1 gültiger Kandidat → 1/1", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => okResponse(JSON.stringify({ candidates: [cand("beruf", "Arbeitet als Koch.")] }))));
    const r = await run();
  ...
  it("F: Kandidat vor dem Bereinigen, danach 0 → 1/0", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => okResponse(JSON.stringify({ candidates: [cand("x", "y")] }))));
    const r = await run();
  ...
  it("G: mehrere Kandidaten → 3/2 (Duplikat verworfen)", async () => {
    const body = { candidates: [cand("beruf", "Koch."), cand("beruf", "Doppelt."), cand("stadt", "Berlin.")] };
    vi.stubGlobal("fetch", vi.fn(async () => okResponse(JSON.stringify(body))));
    const r = await run();
  ...
  it("Security: Protokollzeile enthält nur technische Felder", () => {
    ...
    spy.mockRestore();
    for (const forbidden of [SECRET, "GEHEIMER_USERTEXT", "GEHEIME_MEMORY", "Koch", "Bearer", "user_id", "content", "prompt"]) {
      expect(line).not.toContain(forbidden);
    }
    const json = JSON.parse(line.replace("[orb.obs.analysis_run] ", ""));
    expect(Object.keys(json).sort()).toEqual(
      ["analysis_run_id", "duration_ms", "failure_kind", "http_status", "post_sanitize_count", "pre_sanitize_count", "provider"].sort(),
    );
  });
});
