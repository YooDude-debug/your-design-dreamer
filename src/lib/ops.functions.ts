...
export const opsSelfTest = createServerFn({ method: "POST" })
  ...
  .inputValidator((input) =>
    z
      .object({
        scenario: z.enum(["api_error", "webhook_error", "push_failure", "high_latency", "db_error"]),
      })
      .parse(input),
  )
  .handler(async ({ context, data }): Promise<{ ok: boolean; blocked?: string; events: number }> => {
    const { assertAdmin } = await import("@/lib/admin.server");
    await assertAdmin(context);
    const { appEnvironment } = await import("@/lib/environment.server");
    ...
    const plans: Record<
      ...
      { area: "api" | "webhook" | "push" | "performance" | "database"; event: string; times: number }
    > = {
