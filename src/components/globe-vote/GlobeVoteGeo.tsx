  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5">
        <label className="control-field flex min-w-0 flex-1 items-center gap-2 rounded-xl px-2.5 py-2">
        ...
        <button
          ...
          className="tap-safe shrink-0 rounded-full border border-brand/50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-brand disabled:opacity-50"
        >
          {at.geoSearchBtn}
        </button>
      </div>
      ...
      {results && (
        <ul className="divide-y divide-border/60 overflow-hidden rounded-xl border border-border/60">
          {results.length === 0 ? (
            <li className="px-2.5 py-2 text-[11px] text-muted-foreground">{at.geoNoResults}</li>
          ) : (
            results.map((r) => (
              <li key={`${r.latitude},${r.longitude}`}>
                <button
                  ...
                  className="tap-safe block w-full px-2.5 py-2 text-left text-[11px] hover:bg-brand/10"
                >
      ...
      {point && (
        <div className="rounded-xl border border-brand/40 bg-brand/5 px-2.5 py-2">
          <p className="flex items-center gap-1.5 text-xs">
            <MapPin className="h-3.5 w-3.5 shrink-0 text-brand" />
            <span className="min-w-0 truncate">
      ...
      <input
        ...
        className="w-full rounded-lg border border-border bg-background px-2.5 py-2 text-xs outline-none focus:border-brand/60"
      />
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          disabled={busy || !point}
          onClick={() => void save()}
          className="tap-safe rounded-full border border-brand/50 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-brand disabled:opacity-50"
        >
          {at.geoSaveBtn}
        </button>
        <button
          ...
          className="tap-safe rounded-full border border-border px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground disabled:opacity-50"
        >
          {at.geoCancelBtn}
        </button>
      </div>
