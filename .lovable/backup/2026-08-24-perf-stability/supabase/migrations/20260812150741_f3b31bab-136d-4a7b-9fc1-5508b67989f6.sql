-- 1) SECURITY DEFINER: EXECUTE nur für die tatsächlich benötigten Rollen
REVOKE ALL ON FUNCTION public.guard_admin_role_changes() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.owns_slang_name(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.owns_slang_name(text) TO authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.upsert_slang_definition(uuid, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.upsert_slang_definition(uuid, text, text) TO authenticated, service_role;

-- 2) can_read_media: Pfad muss im Ordner des jeweiligen Eigentümers liegen
