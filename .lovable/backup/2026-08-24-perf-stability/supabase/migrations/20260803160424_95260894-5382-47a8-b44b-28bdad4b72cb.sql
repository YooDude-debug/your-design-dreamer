...
BEGIN
  ...
  IF NEW.kind = 'creator' THEN
    -- Entwicklungsphase: Brand-/Creator-SlangTags duerfen ausschliesslich
    -- Administratoren erstellen bzw. aendern. Creator- und Unternehmer-Rollen
    -- sind vorbereitet, aber noch nicht freigeschaltet.
    IF TG_OP = 'INSERT' OR OLD.kind IS DISTINCT FROM 'creator' THEN
      IF NOT (public.has_role(auth.uid(), 'admin') OR public.has_role(NEW.owner_id, 'admin')) THEN
        RAISE EXCEPTION 'Brand-/Creator-SlangTags koennen derzeit nur von Administratoren erstellt werden';
      END IF;
    END IF;
